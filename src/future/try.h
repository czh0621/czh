#if !defined(_FUTURE_TRY_H)
#define _FUTURE_TRY_H
#include <exception>
#include <iostream>
#include <stdexcept>
#include <string>
...
class Uninitialization : public TryException {
 public:
  Uninitialization() : TryException("Uninitialization Try"){};
};
...
class Try {
 ...
 public:
  ...
  Try(std::exception_ptr e) : m_state(State::Exception), m_exception(e) {}
  ...
  Try(Try<T> &&t) : m_state(std::move(t.m_state)) {
    if (t.m_state == State::Exception) {
  ...
  const std::exception_ptr &exception() const & {
    if (!has_exception()) {
      std::runtime_error("Not has exception! State error!");
    }
    return m_exception;
  }
...
class Try<void> {
 ...
 public:
  ...
  explicit Try(std::exception_ptr e)
      : m_exception(e), m_state(State::Exception) {}
  ...
  Try(Try<void> &&t) : m_state(std::move(t.m_state)) {
    if (t.m_state == State::Exception) {
      new (&m_exception) std::exception_ptr(std::move(t.m_exception));
    }
  }

  const Try<void> &operator=(const Try<void> &t) {
    if (&t == this) {
      return *this;
    }
    this->destroy();
...
auto invoke_with_TryWrapper(F &&f, Args &&...args) -> typename std::enable_if<
    ...
    typename TryWrapper<typename std::result_of<F(Args...)>::type>::type>::
    type {
  try {
    using return_type = typename std::result_of<F(Args...)>::type;
    return TryWrapper<return_type>::type(
        std::forward<F>(f)(std::forward<Args...>(args...)));
  } catch (const std::exception &e) {
    (void)e;
    return TryWrapper<typename std::result_of<F(Args...)>::type>::type(
...
template <typename F>
auto invoke_with_TryWrapper(F &&f, Try<void> &&arg) -> typename std::enable_if<
    !std::is_same<typename std::result_of<F()>::type, void>::value,
    typename TryWrapper<typename std::result_of<F()>::type>::type>::type {
  try {
    using return_type = typename std::result_of<F()>::type;
    return TryWrapper<return_type>::type(std::forward<F>(f)());
  } catch (const std::exception &e) {
    (void)e;
    return TryWrapper<typename std::result_of<F()>::type>::type(
        std::current_exception());
  }
}
