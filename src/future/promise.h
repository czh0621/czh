...
#ifndef CZH_PROMISE_H_
#define CZH_PROMISE_H_
...
public:
    ...
    template<typename SHIT = T,
             typename      = typename std::enable_if<!std::is_void<SHIT>::value, SHIT>::type>
    ...
    // 这里采用函数模板来重载，这种方法可读性更好,根本原因是void类型与T&& 不匹配,void类型没有copy 和
    // move
    ...
    Future<T> get_future()
    {
        ...
        if (!m_shared_state->m_retrived.compare_exchange_strong(expect, true)) {
            throw std::runtime_error("promise value has been retrieved!");
        }
