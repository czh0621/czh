...
#ifndef CZH_SHARED_STATE_H_
#define CZH_SHARED_STATE_H_
#include <memory.h>
...
public:
    ...
    FutureStatus wait_until(const std::chrono::duration<Clock, Duration>& timeout_time)
    {
        std::unique_lock<std::mutex> lck(m_then_lck);
        m_blocked = true;
    ...
    FutureStatus wait_for(const std::chrono::duration<Rep, Duration>& timeout_duration)
    {
        std::unique_lock<std::mutex> lck(m_then_lck);
        m_blocked = true;
        bool ret  = m_condition.wait_for(
            lck, timeout_duration, [this]() { return m_status != FutureStatus::None; });
        if (!ret) {
            m_status = FutureStatus::Timeout;
        }
        return m_status;
